...
GrowthSW <- GrowthSW %>% mutate(rev_or_not = if_else(revolutions > 0, 'treat', '0'))

datasummary(growth + rgdp60 ~ factor(rev_or_not) * (Mean + median + sd + min + max),
            data = GrowthSW)
...
```{r}
# write t test here
t.test(GrowthSW$growth, GrowthSW$rev_or_not, 
                    alternative = c("two.sided"),
                    mu = 100 , paired = TRUE, var.equal = FALSE,
                    conf.level = 0.95)
