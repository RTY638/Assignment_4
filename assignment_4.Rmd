...
author: |
  | Roan ter Huurne
  | r.t.y.terhuurne@students.uu.nl
  | http://www.github.com/RTY638
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
GrowthSW <- GrowthSW %>% mutate(treat = if_else(revolutions == 0, "revolutions equal to 0", "revolutions greater than 0" ))
...
**Designated place**: Looking at the table it becomes evident that in countries without revolutions the growth is more seen than with revolutions. It can be observed by looking at values of median and mean(2.29 and 2.46)
...
the p-value is 0.06871. this means that it falls inside the 95% confidence interval that was set for the t-test. this then means that the original hypothesis is not rejected which is that growth rates are affected by revolutions.
...
including the variable `rgdp60` is purposefully done to have some form of benchmark to the value of currency. if this wasn't included there is no way to pair growth to some other variable and conclude a difference.
...
according to this analysis the main driver of economic growth is tradeshare, this is because its coeficcients are way higher than the other variables. this means that for an increase in tradeshare growth increases more relative to an increase, by the same amount, in a different factor. 
